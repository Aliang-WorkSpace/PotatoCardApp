...
final class WeatherAutoUpdateScheduler: ObservableObject {
    ...
    private func runIfDue(reason: String) async -> Bool {
        ...
        guard wasLastProjectedContentWeather(configuration: store.config) else {
            logger.info("天气自动更新跳过：上次投屏内容不是天气。")
            return true
        }
    ...
    private func wasLastProjectedContentWeather(configuration: WeatherSkillConfiguration) -> Bool {
        let snapshotDeviceID = configuration.targetDeviceSnapshot?.id.trimmed ?? ""
        let targetDeviceID = snapshotDeviceID.isEmpty ? configuration.targetDeviceID.trimmed : snapshotDeviceID
        guard !targetDeviceID.isEmpty else { return false }
        return ProjectedContentStore().loadRecord(for: targetDeviceID)?.type == .weather
    }
